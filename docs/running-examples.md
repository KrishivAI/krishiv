...
cargo build -p krishiv --profile release-k8s --target x86_64-unknown-linux-musl

# 2. Strip debug symbols (reduces image from ~400 MB to ~120 MB)
strip target/x86_64-unknown-linux-musl/release-k8s/krishiv

# 3. Stage the binary and write the Dockerfile
mkdir -p /tmp/krishiv-image
cp target/x86_64-unknown-linux-musl/release-k8s/krishiv /tmp/krishiv-image/
