...
- `submit_sql_job` runs through a session background registry while preserving
  ...
  submissions and still uses the configured coordinator endpoint for
  distributed jobs.
...
- `register_sink` validates connector sink configs through the built-in
  connector registry and stores them as session metadata. SQL job execution
  still uses explicit `CREATE SINK ... INTO ...` statements until registered
  sink names are resolved by the SQL/pipeline compiler.
- Generic source registration directly supports Parquet through
  `Session::register_parquet_async`; other connectors should be declared through
  SQL `CREATE SOURCE` and submitted with the job tools until generic source
  registration is exposed through `Session`.
...
`cargo test --workspace --exclude krishiv-python --exclude krishiv-chaos --no-fail-fast`
outside the restricted sandbox, or continue MCP connector work by resolving
registered source/sink names in the SQL pipeline compiler.
