...
## Phase 5: open-source readiness (2026-06-12)

### Done

- Rewrote `docs/architecture.md` around the implemented engine boundary, crate ownership, runtime lifecycle, durability, security, and extension rules.
- Added public roadmap, governance, conduct, security, compatibility, connector SDK, benchmarking, release, changelog, license, and ADR documentation.
- Added structured issue forms, a contract-aware pull-request template, offline documentation/release checks, benchmark manifests, and CI/release workflows.
...
python3 scripts/benchmark_manifest.py --suite script-smoke --command true --output /tmp/krishiv-benchmark-manifest.json  # passed
cargo fmt --all --check                          # passed
git diff --check                                 # passed
```

`just project-check` could not run in this container because `just` is not installed; its three constituent commands passed directly.

### Blockers

The repository has no configured Git remote in this checkout, so project URLs, named maintainers, and private security contacts cannot be verified here.

### Next useful task

Configure repository ownership, `CODEOWNERS`, private vulnerability reporting, signed provenance/SBOM generation, and permanent benchmark history.
