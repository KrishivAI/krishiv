...
**R13 Python API gaps (2026-05-23).** Previously blocked Python surface area is implemented on branch `cursor/python-r13-api-7aa2`.

### Python blocked items — implemented

| Item | Status |
|------|--------|
| `ks.Schema` / type-annotation → Arrow | `Schema` + `__init_subclass__` in `krishiv-python/src/schema.rs` |
| `read_iceberg()` | `read_iceberg()` with `iceberg` Cargo feature + `MemoryLakehouseTable` probe |
| `.with_watermark()`, `.key_by()`, `.window()`, `.agg()` | `Stream` / `KeyedStream` / `WindowedStream` + `windows` / `agg` submodules |
| `batch.to_pandas()`, `batch.to_arrow()` | `PyBatch` holds `RecordBatch`; `pyo3-arrow` export |
| Jupyter `_repr_html_()` | `Batch`, `Stream`, `Schema` |
| `krishiv[kafka]`, `krishiv[iceberg]` extras | `pyproject.toml` optional-deps; native `--features kafka\|iceberg` |
| `@ks.state_migration` (R16) | `state_migration` decorator + `register_state_migration` / `apply_state_migration` |

**Validation:** `cargo test -p krishiv-python --features iceberg` · `pytest crates/krishiv-python/python/tests/` (after `maturin develop --features iceberg`)
