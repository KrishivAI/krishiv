...
- [x] **IVM-AUD-PERF-1: the tick was O(accumulated rows)** — DONE
  ...
  See `docs/engineering-log/ivm-audit-register.md` PERF-1.
...
- `docs/guides/running-examples.md` — `--profile release-k8s` → `--profile release`,
...
## 2026-09-05 — documentation rewrite (architecture set)

- **Completed**: replaced the planning-era documentation tree with a
  19-document architecture reference (`docs/architecture/00`–`18`) covering
  every crate and feature; `docs/README.md` is now the index. Active
  registers and dated evidence moved to `docs/engineering-log/`; superseded
  phase plans, specs, roadmaps, design notes and `docs/superpowers/` were
  deleted (recoverable from git). `ci-tiers.md` and `async-contract.md` were
  folded into `17-testing-and-quality.md`, `feature-graph.md` into
  `15-configuration.md`; every code/CI/justfile reference repointed. All 23
  crate READMEs rewritten (several described the wrong crate, e.g.
  `krishiv-delta` as Delta Lake). Root `README.md` quick starts corrected to
  the real Python/Rust API. Certification matrix re-blessed for the moved
  IVM register path.
- **Validation**: `python3 scripts/check_markdown_links.py .` (all links
  resolve), `cargo fmt --all -- --check`, `KRISHIV_BLESS_CERT_MATRIX=1 cargo
  test -p krishiv-connectors cert_matrix` (4 passed).
- **Blocker (pre-existing, not fixed)**: `just project-check` fails at
  `scripts/check_api_surface.py` — `api/*.json` and `api/rust-public-api.txt`
  are stale on `HEAD` independent of this work (verified in a clean worktree).
  Regenerating with `--write` is a 5.9k-line diff and should be its own
  reviewed commit.
