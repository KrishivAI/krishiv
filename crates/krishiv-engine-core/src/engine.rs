...
pub trait ComputeEngine: Send + Sync {
    ...
    /// Execute `job` using the placement-provided services in `rt`.
    ///
    /// `run` is the **bounded** invocation: every engine drains its source(s)
    /// to end-of-input and returns a handle whose status is
    /// [`JobStatus::Completed`] — nothing continues after it returns. Unbounded
    /// continuous maintenance is a separate driver (`spawn_streaming_job`),
    /// whose handle reports [`JobStatus::Running`] until it is stopped.
