...
/// # Memory management
///
/// Entries accumulate without bound unless callers invoke [`forget`] /
/// [`forget_many`] after retracting the associated outputs, or call
/// [`gc_before_epoch`] with a committed watermark epoch.  Long-running jobs
/// that never call either will leak memory proportional to the total number of
/// unique input rows ever processed.
#[derive(Debug, Default)]
pub struct ProvenanceIndex {
    input_to_outputs: AHashMap<u64, AHashSet<u64>>,
    /// Epoch at which each input hash was first recorded.  Populated only when
    /// callers use [`record_with_epoch`]; zero-cost when unused.
    input_epochs: AHashMap<u64, u64>,
}
...
impl ProvenanceIndex {
    ...
    /// Record that `input_hash` produced `output_hash` at `epoch`.
    ///
    /// When [`gc_before_epoch`](Self::gc_before_epoch) is called with a
    /// committed watermark, entries whose epoch is strictly less than the
    /// watermark are evicted automatically.
    pub fn record_with_epoch(&mut self, input_hash: u64, output_hash: u64, epoch: u64) {
        self.input_to_outputs
            .entry(input_hash)
            .or_default()
            .insert(output_hash);
        self.input_epochs.entry(input_hash).or_insert(epoch);
    }
    ...
    /// Evict all provenance entries recorded at an epoch strictly less than
    /// `committed_epoch`.  Call this after each successfully committed IVM
    /// delta epoch to bound memory growth.
    ///
    /// Only entries added via [`record_with_epoch`](Self::record_with_epoch)
    /// carry epoch metadata; plain [`record`](Self::record) entries are never
    /// evicted by this method (use [`forget`](Self::forget) for those).
    pub fn gc_before_epoch(&mut self, committed_epoch: u64) {
        let to_evict: Vec<u64> = self
            .input_epochs
            .iter()
            .filter(|(_, ep)| **ep < committed_epoch)
            .map(|(h, _)| *h)
            .collect();
        for h in to_evict {
