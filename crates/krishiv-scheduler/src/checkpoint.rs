...
impl CheckpointCoordinator {
    ...
    ///
    /// If the epoch hint file is stale (e.g. coordinator crashed between
    /// `write_epoch_metadata` and `write_epoch_hint`), `latest_valid_epoch`
    /// falls back to scanning `list_valid_epochs`.  A warn! is emitted in that
    /// case so operators can detect an incomplete prior commit.
    pub fn recover_from_storage(&mut self) -> CheckpointResult<Option<u64>> {
        match latest_valid_epoch(self.storage.as_ref(), self.job_id.as_str()) {
            Ok(epoch) => {
                ...
                {
                    // Warn if the stored fencing token differs from ours — this
                    // indicates the recovered epoch was committed by a prior
                    // coordinator generation (coordinator crashed mid-commit and a
                    // newer generation completed it).  Stale executor acks for the
                    // in-flight epoch are rejected via the fencing token on arrival.
