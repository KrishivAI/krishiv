...
impl Coordinator {
    ...
    /// Create a new active coordinator (P3.11: canonical two-constructor form).
    pub fn new_active(config: Option<CoordinatorConfig>) -> Self {
        let id = CoordinatorId::try_new("coordinator-1").expect("static coordinator id");
        Self::build(id, config.unwrap_or_default(), CoordinatorState::Active)
    }

    /// Create a new standby coordinator (P3.11: canonical two-constructor form).
    pub fn new_standby(config: Option<CoordinatorConfig>) -> Self {
        let id = CoordinatorId::try_new("coordinator-1").expect("static coordinator id");
        Self::build(id, config.unwrap_or_default(), CoordinatorState::Standby)
    }
