...
fn session_window_separate_keys_independent() {
    ...
    // flush_closed_sessions packs all closed sessions into one multi-row batch.
    let total_rows: usize = out.iter().map(|b| b.num_rows()).sum();
    assert_eq!(total_rows, 2, "each key's session must close independently");
}
...
/// Latency regression: a `ContinuousWindowExecutor` drains 100 batches of 1k
/// rows each (100k events) and emits the closed window in well under 1 s.
///
/// The per-batch inner loop is the hot path: it looks up the row's key and
/// event-time, hashes the key, and updates the per-key accumulator. Before the
/// column-index caching + watermark-caching optimizations, this took ~1.5 s
/// on a typical CI box. The 1 s budget here is conservative; the actual floor
/// is dominated by `HashMap` insert/lookup and Arrow array walks, both of
/// which are unchanged. The optimization saves two `HashMap<String, usize>`
/// lookups per batch (the `index_of` cache) and ~20 ns of `i128` arithmetic
/// per row (the watermark cache), which on 100k rows adds up to milliseconds
/// but not seconds. The 1 s ceiling still catches any future regression
/// (e.g. accidentally re-introducing a per-batch allocation on the hot path).
#[test]
fn continuous_window_drain_100k_rows_under_1s() {
    ...
    // 100 batches of 1k rows each, spread over 100 keys.
    let mut spec = WindowExecutionSpec::tumbling("user_id", "ts", 10_000);
    spec.watermark_lag_ms = 0;
    let mut exec = ContinuousWindowExecutor::new(spec).unwrap();

    let start = Instant::now();
    for batch_idx in 0..100 {
        let user_ids: Vec<String> = (0..1000).map(|i| format!("user_{}", i % 100)).collect();
        let user_id_refs: Vec<&str> = user_ids.iter().map(|s| s.as_str()).collect();
        let ts_values: Vec<i64> = (0..1000).map(|i| (batch_idx * 1000 + i) as i64).collect();
        let v_values: Vec<i64> = (0..1000).map(|i| i as i64).collect();
        let arr_user: arrow::array::ArrayRef =
            Arc::new(arrow::array::StringArray::from(user_id_refs));
        let arr_ts: arrow::array::ArrayRef = Arc::new(arrow::array::Int64Array::from(ts_values));
        let arr_v: arrow::array::ArrayRef = Arc::new(arrow::array::Int64Array::from(v_values));
        let batch = RecordBatch::try_new(schema.clone(), vec![arr_user, arr_ts, arr_v]).unwrap();
        let _ = exec.drain(vec![batch]).expect("drain");
    }
    let elapsed = start.elapsed();
    assert!(
        elapsed.as_secs() < 1,
        "100 batches of 1k rows should drain in <1s, took {elapsed:?}"
    );
}
