...
pub struct OperatorQueueSender {
    ...
    /// `Mutex`'s contents. The `Mutex` is never read through this field
    /// path — the field is a lifetime-extension placeholder.
    #[expect(
        dead_code,
        reason = "held for Arc lifetime extension; Mutex contents read via receiver's path"
    )]
