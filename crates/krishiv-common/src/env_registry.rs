...
const fn rt(name: &'static str, kind: FlagKind, default: &'static str, doc: &'static str) -> FlagSpec {
    FlagSpec { name, kind, default, doc, scope: FlagScope::Runtime }
}

const fn test(name: &'static str, kind: FlagKind, default: &'static str, doc: &'static str) -> FlagSpec {
    FlagSpec { name, kind, default, doc, scope: FlagScope::Test }
}

const fn bench(name: &'static str, kind: FlagKind, default: &'static str, doc: &'static str) -> FlagSpec {
    FlagSpec { name, kind, default, doc, scope: FlagScope::Bench }
}
...
pub static FLAGS: &[FlagSpec] = &[
    rt("KRISHIV_ALLOW_ANONYMOUS", FlagKind::Bool, "false",
    ...
    rt("KRISHIV_MATCH_RECOGNIZE_STREAMING_LIMIT", FlagKind::UInt, "engine default",
       "Row cap for MATCH_RECOGNIZE evaluation over streaming inputs."),
    rt("KRISHIV_MAX_CONCURRENT_ASSIGNMENT_RPCS", FlagKind::UInt, "16",
       "Coordinator-side concurrency cap for task assignment RPC fan-out."),
    rt("KRISHIV_MCP_ADDR", FlagKind::SocketAddr, "127.0.0.1:8811",
       "MCP server listen address (http transport)."),
    rt("KRISHIV_MCP_ALLOW_WRITE_SQL", FlagKind::Bool, "false",
       "Allow the MCP run_sql tool to execute write statements."),
    rt("KRISHIV_MCP_MAX_ROWS", FlagKind::UInt, "1000",
       "Row cap on MCP query results."),
    rt("KRISHIV_MCP_TIMEOUT_MS", FlagKind::UInt, "30000",
       "MCP tool execution timeout."),
    rt("KRISHIV_MCP_TRANSPORT", FlagKind::Enum(&["stdio", "http"]), "stdio",
       "MCP server transport."),
    rt("KRISHIV_METADATA_BACKEND", FlagKind::Enum(&["memory", "rocksdb", "redb"]), "rocksdb",
       "Coordinator metadata store backend."),
    rt("KRISHIV_METADATA_PATH", FlagKind::Path, "unset",
       "Filesystem path for the persistent coordinator metadata store."),
    rt("KRISHIV_MODE", FlagKind::Enum(&["embedded", "single-node", "distributed", "bare-metal", "k8s"]), "embedded",
       "Session execution mode selector."),
    rt("KRISHIV_NAMESPACE", FlagKind::Text, "default",
       "Kubernetes namespace the operator manages."),
    rt("KRISHIV_NAMESPACE_MAX_ACTIVE_JOBS", FlagKind::UInt, "unset",
       "Admission cap: maximum concurrently active jobs per namespace."),
    rt("KRISHIV_NAMESPACE_MAX_CPU_NANOS", FlagKind::UInt, "unset",
       "Admission cap: maximum aggregate CPU (nanos) per namespace."),
    rt("KRISHIV_NAMESPACE_MAX_MEMORY_BYTES", FlagKind::UInt, "unset",
       "Admission cap: maximum aggregate memory per namespace."),
    rt("KRISHIV_OIDC_AUDIENCE", FlagKind::Text, "unset",
       "Expected audience claim for OIDC-authenticated coordinator requests."),
    rt("KRISHIV_OIDC_JWKS_URI", FlagKind::Url, "unset",
       "JWKS endpoint for OIDC token verification; presence activates OIDC auth."),
    rt("KRISHIV_PLAN_CACHE_MAX_ENTRIES", FlagKind::UInt, "128",
       "Logical-plan cache capacity per SQL session."),
    rt("KRISHIV_PRODUCTION", FlagKind::Bool, "false",
       "Production mode: tightens defaults (fail-closed metadata, auth requirements, connector restrictions)."),
    rt("KRISHIV_PYTHON_UDF_TIMEOUT_MS", FlagKind::UInt, "30000",
       "Per-call timeout for sandboxed Python UDF execution."),
    rt("KRISHIV_QUERY_MEMORY_LIMIT_BYTES", FlagKind::UInt, "cgroup-derived",
       "Per-query FairSpillPool budget for embedded/IVM sessions."),
    rt("KRISHIV_REMOTE_EXEC", FlagKind::Bool, "mode-dependent",
       "Force remote (coordinator) execution on or off for API sessions."),
    rt("KRISHIV_REQUIRE_EXECUTOR_TASK_AUTH", FlagKind::Bool, "profile-dependent",
       "Require bearer auth on executor task gRPC even in dev profiles."),
    rt("KRISHIV_RESULT_SPOOL_DIR", FlagKind::Path, "temp dir",
       "Directory for disk-spooled large query results."),
    rt("KRISHIV_RESULT_SPOOL_MAX_BYTES", FlagKind::UInt, "1073741824",
       "Cap on total spooled result bytes per node."),
    rt("KRISHIV_ROCKSDB_MAX_OPEN_FILES", FlagKind::Int, "rocksdb default",
       "RocksDB max_open_files for state/metadata stores (-1 = unlimited)."),
    rt("KRISHIV_ROCKSDB_WRITE_BUFFER_MB", FlagKind::UInt, "rocksdb default",
       "RocksDB write-buffer (memtable) size in MiB."),
    rt("KRISHIV_SHUFFLE_ADDR", FlagKind::SocketAddr, "127.0.0.1:50060",
       "Shuffle service HTTP listen address."),
    rt("KRISHIV_SHUFFLE_DIR", FlagKind::Path, "temp dir",
       "Local-disk shuffle store directory."),
    rt("KRISHIV_SHUFFLE_FETCH_CONCURRENCY", FlagKind::UInt, "4",
       "Reduce-side concurrent shuffle partition fetches."),
    rt("KRISHIV_SHUFFLE_FETCH_RETRIES", FlagKind::UInt, "3",
       "Retry attempts per shuffle partition fetch."),
    rt("KRISHIV_SHUFFLE_FETCH_RETRY_BASE_MS", FlagKind::UInt, "100",
       "Base backoff for shuffle fetch retries."),
    rt("KRISHIV_SHUFFLE_FLIGHT_ADDR", FlagKind::SocketAddr, "unset",
       "Shuffle Flight transport listen address (executor)."),
    rt("KRISHIV_SHUFFLE_MEMORY_BYTES", FlagKind::UInt, "268435456",
       "In-memory shuffle store budget before spill/rejection."),
    rt("KRISHIV_SHUFFLE_PARTITIONS", FlagKind::UInt, "target-parallelism",
       "Default shuffle partition count for distributed plans."),
    rt("KRISHIV_SHUFFLE_SPILL_THRESHOLD_BYTES", FlagKind::UInt, "67108864",
       "Sort-shuffle writer in-memory buffer threshold before spilling a run."),
    rt("KRISHIV_SHUFFLE_TOKEN", FlagKind::Secret, "unset",
       "Bearer token protecting shuffle service endpoints."),
    rt("KRISHIV_SHUFFLE_TOKEN_FILE", FlagKind::Path, "unset",
       "File containing the shuffle bearer token; hot-reloaded."),
    rt("KRISHIV_SHUFFLE_TOKEN_RELOAD_SECS", FlagKind::UInt, "30",
       "Interval for re-reading the shuffle token file."),
    rt("KRISHIV_SHUFFLE_URI", FlagKind::Url, "unset",
       "Shuffle backend URI (file://, s3://, tiered://local;s3://…)."),
    rt("KRISHIV_STATE_DIR", FlagKind::Path, "unset",
       "Executor state-backend directory (RocksDB window/operator state)."),
    rt("KRISHIV_STREAMING_TASK_TIMEOUT_SECS", FlagKind::UInt, "unset",
       "Watchdog timeout for streaming task cycles; unset = disabled."),
    rt("KRISHIV_STREAM_EARLY_FIRE_MS", FlagKind::UInt, "unset",
       "Speculative early-fire interval for open windows (embedded loop; distributed wiring is Phase 55)."),
    rt("KRISHIV_STREAM_PROFILE", FlagKind::Enum(&["low-latency", "throughput"]), "low-latency",
       "Embedded streaming loop profile (checkpoint cadence, batching)."),
    rt("KRISHIV_TARGET_PARALLELISM", FlagKind::UInt, "cores",
       "DataFusion target partition count for local execution."),
    rt("KRISHIV_TASK_GRPC_ADDR", FlagKind::SocketAddr, "127.0.0.1:50052",
       "Executor task gRPC listen address."),
    rt("KRISHIV_TASK_SLOTS", FlagKind::UInt, "CPU-derived",
       "Executor task slots; unset derives from available CPU cores."),
    rt("KRISHIV_TLS_CERT", FlagKind::Path, "unset",
       "TLS certificate path for coordinator/executor gRPC servers."),
    rt("KRISHIV_TLS_KEY", FlagKind::Path, "unset",
       "TLS private-key path for coordinator/executor gRPC servers."),
    rt("KRISHIV_UI_TOKEN", FlagKind::Secret, "unset",
       "Bearer token protecting the embedded web UI."),
    rt("KRISHIV_UI_TOKEN_FILE", FlagKind::Path, "unset",
       "File containing the UI bearer token."),
    rt("KRISHIV_UNITY_CATALOG_NAME", FlagKind::Text, "main",
       "Catalog name to register the Unity Catalog integration under."),
    rt("KRISHIV_UNITY_HOST", FlagKind::Url, "unset",
       "Unity Catalog host URL; presence activates the integration."),
    rt("KRISHIV_UNITY_TOKEN", FlagKind::Secret, "unset",
       "Bearer token for Unity Catalog."),
    rt("KRISHIV_WAREHOUSE_ROOT", FlagKind::Path, ".",
       "Root path for connector-table warehouse storage."),
    // ── Test-scope flags ────────────────────────────────────────────────
    test("KRISHIV_KIND_CLUSTER", FlagKind::Text, "krishiv-e2e",
         "kind cluster name for operator e2e smoke tests."),
    test("KRISHIV_KIND_E2E", FlagKind::Bool, "false",
         "Enable the kind-based operator e2e smoke tests."),
    test("KRISHIV_KIND_IMAGE", FlagKind::Text, "unset",
         "Engine image to load into the kind cluster."),
    test("KRISHIV_KIND_NAMESPACE", FlagKind::Text, "default",
         "Namespace used by kind e2e tests."),
    test("KRISHIV_KIND_SKIP_CREATE", FlagKind::Bool, "false",
         "Reuse an existing kind cluster instead of creating one."),
    test("KRISHIV_KIND_SKIP_LOAD_IMAGE", FlagKind::Bool, "false",
         "Skip loading the engine image into kind."),
    test("KRISHIV_KIND_TIMEOUT_SECS", FlagKind::UInt, "300",
         "Timeout for kind e2e operations."),
    test("KRISHIV_TEST_DATABASE_URL", FlagKind::Url, "unset",
         "Postgres URL for catalog integration tests."),
    test("KRISHIV_TEST_S3_BUCKET", FlagKind::Text, "unset",
         "S3 bucket for object-store integration tests."),
    // ── Bench-scope flags ───────────────────────────────────────────────
    bench("KRISHIV_TPCDS_DATA_DIR", FlagKind::Path, "unset",
          "TPC-DS dataset directory for the bench harness."),
    bench("KRISHIV_TPCH_DATA_DIR", FlagKind::Path, "unset",
          "Legacy TPC-H SF10 dataset directory (prefer the _SF* variants)."),
    bench("KRISHIV_TPCH_DATA_DIR_SF1", FlagKind::Path, "unset",
          "TPC-H SF1 dataset directory."),
    bench("KRISHIV_TPCH_DATA_DIR_SF10", FlagKind::Path, "unset",
          "TPC-H SF10 dataset directory."),
    bench("KRISHIV_TPCH_DATA_DIR_SF100", FlagKind::Path, "unset",
...
pub enum EnvIssue {
    ...
    Invalid { name: String, kind: &'static str, value_hint: String },
    /// A deprecated alias is set; the canonical name should be used.
    DeprecatedAlias { name: String, canonical: &'static str },
}
...
impl std::fmt::Display for EnvIssue {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::Invalid { name, kind, value_hint } => {
...
pub fn reference_markdown() -> String {
    ...
    for (scope, title) in [
        ...
        for f in FLAGS.iter().filter(|f| f.scope == scope) {
            ...
            let _ = writeln!(out, "| `{}` | {} | `{}` | {} |", f.name, kind, f.default, f.doc);
        }
    }
    out.push_str("\n## Dynamic namespaces\n\n");
...
mod tests {
    ...
    /// Meta-flags of the registry test machinery itself — read only below.
    const SCAN_ALLOWLIST: &[&str] = &["KRISHIV_BLESS_ENV_REFERENCE"];
    ...
    fn every_flag_read_in_source_is_declared() {
        ...
        let undeclared: Vec<_> = seen
            ...
            .filter(|name| {
                ...
                lookup(name).is_none()
                    && !FLAG_PREFIXES.iter().any(|(p, _)| p.trim_end_matches('_') == name.trim_end_matches('_') || name.starts_with(p))
            })
            .collect();
        assert!(
            undeclared.is_empty(),
