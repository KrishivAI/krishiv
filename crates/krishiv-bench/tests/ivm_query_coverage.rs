...
/// remaining 10, measured from the printed decorrelated shapes: q2's scalar
/// side reads FOUR tables (a side that is itself a join run — sides are
/// single-table in v1); q11, q15 and q22 keep UNCORRELATED scalar
/// subqueries the optimizer rules leave as raw Subquery exprs; q20's
/// membership side contains its own nested scalar join; q21's semi level
/// carries a non-equi membership conjunct (`l_suppkey !=`); self-joins
/// collide bare names (q7, q8, and q21's triple lineitem); q13 is a LEFT
/// OUTER aggregate; q19's equi keys hide inside an OR.
...
/// though they were.
const TPCH_INCREMENTAL: usize = 12;
...
const TPCH_DECOMPOSED: usize = 12;
