...
/// One query: stable id, the SQL, and the tables it reads.
///
...
pub static TPCH_QUERIES: &[TpchQuery] = &[
    ...
    TpchQuery {
        ...
        sql: "SELECT o_year, sum(CASE WHEN nation = 'BRAZIL' THEN volume ELSE 0 END) / sum(volume) AS mkt_share \
              FROM ( \
    ...
    TpchQuery {
        ...
        tables: &["customer", "orders"],
        sql: "SELECT cntrycode, count(*) AS numcust, sum(c_acctbal) AS totacctbal FROM ( \
