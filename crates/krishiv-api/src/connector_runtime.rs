...
use krishiv_engine_core::{
    ChangelogBatch, CompiledJob, ConsolidatingSinkProvider, DurableCheckpointService, EngineError,
    EngineResult, EngineRuntime, JobHandle, JobStatus, Placement, QueryExecutor, RowKind,
    SinkProvider, SinkSpec, SinkWriter, SourceProvider, SourceReader, SourceSpec,
    UpsertSinkProvider,
};
...
pub async fn run_incremental_job_via_ivm(
    ...
    Ok(JobHandle::from_name(&job.name, JobStatus::Running)?)
}
...
impl QueryExecutor for RuntimeQueryExecutor {
    async fn execute_batch(&self, job: &CompiledJob) -> EngineResult<Vec<RecordBatch>> {
        let mut tables = Vec::with_capacity(job.sources.len());
        for spec in &job.sources {
            if spec.connector != "parquet" {
                return Err(EngineError::Source(format!(
                    "connector '{}' is not available for distributed batch execution yet; \
                     supported: parquet",
                    spec.connector
                )));
            }
...
impl SourceProvider for ConnectorSourceProvider {
    async fn open(&self, spec: &SourceSpec) -> EngineResult<Box<dyn SourceReader>> {
        match spec.connector.as_str() {
            ...
            "json" | "ndjson" => Ok(Box::new(JsonFileSourceReader::open(&spec.uri)?)),
...
impl JsonFileSourceReader {
    fn open(uri: &str) -> EngineResult<Self> {
        let bytes = std::fs::read(uri)
            .map_err(|e| EngineError::Source(format!("read json source '{uri}': {e}")))?;
        let inner = krishiv_connectors::csv_json::NdjsonSource::open(bytes, Default::default())
            .map_err(|e| EngineError::Source(e.to_string()))?;
        Ok(Self { inner })
    }
}
...
fn require_append_only(changes: &ChangelogBatch, kind: &str) -> EngineResult<()> {
    if !changes.is_append_only() {
        return Err(EngineError::Sink(format!(
            "{kind} sink is append-only; route retraction-bearing output through a \
             consolidating sink"
        )));
    }
    Ok(())
    ...
    async fn write(&mut self, changes: ChangelogBatch) -> EngineResult<()> {
        if !changes.is_append_only() {
            return Err(EngineError::Sink(
                "connector sinks currently accept insert-only changelogs; \
                 retraction-aware (upsert/CDC) sink writing is the Phase 4 increment"
                    .to_string(),
            ));
        }
        let (batch, _kinds) = changes.into_parts();
