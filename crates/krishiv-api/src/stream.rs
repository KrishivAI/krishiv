...
// A-5 fix: dropping a `Stream` must close its unbounded input. Without
// `Drop`, a Python user that creates a stream and then drops the Python
// reference (e.g. `del stream` after a `with` block) leaves the
// `mpsc::Sender` alive in the session, the channel held open, and the
// background consumer polling forever for batches that will never arrive.
// The fix: `Drop` cancels the input, which takes the `mpsc::Sender` out
// of the `Mutex<Option<Sender>>`. The receiver sees EOF and the
// background loop exits.
//
// The `Drop` runs synchronously in the thread that drops the value. The
// cancel is a `Mutex::lock` + `Option::take` — fast and non-blocking, so
// it's safe to call from any thread including a Python GC finalizer.
...
impl fmt::Debug for Stream {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Stream")
            ...
            .field(
                "input_closed",
                &self.input.as_ref().and_then(|i| i.is_closed().ok()),
            )
...
impl Stream {
    ...
    /// A-8: hard-cancel the stream. The unbounded input is closed (so
    /// consumers see end-of-stream) and any pending records are dropped.
    /// Idempotent; safe to call after `close_input`.
    ///
    /// Distinct from `close_input` in that `close_input` is a graceful
    /// "no more records" signal — the consumer drains queued batches and
    /// sees a clean EOF. `cancel` is "abort right now" — pending batches
    /// in the source's internal buffer are discarded and the stream is
    /// closed immediately.
    pub fn cancel(&self) -> Result<bool> {
        self.unbounded_input()?.cancel();
        Ok(true)
    }
