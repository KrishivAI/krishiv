...
fn local_parquet_partition_parse_valid() {
    let partition =
        krishiv_proto::InputPartition::new("part-1", "local-parquet:my_table:/data/file.parquet");
...
fn local_parquet_partition_parse_malformed_no_colon() {
    let partition = krishiv_proto::InputPartition::new("part-1", "local-parquet:only_table_name");
