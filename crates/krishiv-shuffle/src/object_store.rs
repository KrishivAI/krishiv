...
impl ShuffleStore for ObjectStoreShuffleStore {
    ...
    async fn write_partition(
        ...
        let data_path = self.object_path(&partition.id)?;
        self.store
            .put(&data_path, bytes::Bytes::from(buf).into())
            .await
            .map_err(|e| crate::error::io_err(e.to_string()))?;

        let hash_path = self.hash_object_path(&partition.id)?;
        if let Err(e) = self
            .store
            .put(
                &hash_path,
            ...
            .await
        {
            // The main data write succeeded but the hash sidecar failed.
            // The partition is readable but integrity checks will fail until
            // the hash is successfully written (or a subsequent write retries).
            tracing::warn!(
                job_id = %partition.id.job_id,
                stage_id = %partition.id.stage_id,
                partition = %partition.id.partition,
                data_path = %data_path,
                hash_path = %hash_path,
                error = %e,
                "S3 shuffle: data write succeeded but hash sidecar write failed; \
                 partition is present without an integrity checksum"
            );
            return Err(crate::error::io_err(e.to_string()));
        }
