...
use crate::registry::registry::ConnectorRegistry;
...
pub(crate) fn register_builtin_vector_drivers(registry: &mut ConnectorRegistry) {
    ...
    registry.register_vector_sink(Arc::new(GenericVectorSinkDriver::new(
        ConnectorKind::LanceDb,
    )) as SharedVectorSinkDriver);
    registry.register_vector_sink(Arc::new(GenericVectorSinkDriver::new(
        ConnectorKind::Weaviate,
    )) as SharedVectorSinkDriver);
    registry.register_vector_sink(Arc::new(GenericVectorSinkDriver::new(
        ConnectorKind::Pinecone,
    )) as SharedVectorSinkDriver);
    #[cfg(feature = "qdrant")]
    registry.register_vector_sink(Arc::new(GenericVectorSinkDriver::new(
        ConnectorKind::Qdrant,
    )) as SharedVectorSinkDriver);
    #[cfg(feature = "pgvector")]
    registry.register_vector_sink(Arc::new(GenericVectorSinkDriver::new(
        ConnectorKind::Pgvector,
    )) as SharedVectorSinkDriver);
}
