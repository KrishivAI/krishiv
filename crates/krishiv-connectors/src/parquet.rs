...
impl Sink for ParquetSink {
    ...
    async fn write_batch(&mut self, batch: RecordBatch) -> ConnectorResult<()> {
        ...
        } else if let Some(expected) = &self.schema {
            // Reject schema mismatches eagerly at the write boundary rather than
            // letting ArrowWriter produce a cryptic error or silent corruption.
            let got = batch.schema();
            if got.as_ref() != expected.as_ref() {
                return Err(ConnectorError::Parquet(format!(
                    "schema mismatch in write_batch for '{}': expected {:?}, got {:?}",
                    self.path.display(),
                    expected,
                    got
                )));
