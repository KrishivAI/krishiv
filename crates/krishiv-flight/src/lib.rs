...
//! Shared Krishiv Arrow Flight SQL comment protocol for catalog sync and remote streaming control.
...
impl fmt::Display for FlightError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Transport { message } => write!(f, "flight transport error: {message}"),
        }
    }
}
...
/// Flight protocol result alias.
pub type FlightResult<T> = Result<T, FlightError>;

/// Parquet table registration for batch SQL over Flight.
#[derive(Debug, Clone, PartialEq, Eq)]
...
pub fn encode_continuous_register(job_id: &str, spec: &WindowExecutionSpec) -> String {
    let json = serde_json::to_string(spec).expect("window spec serializes");
    let encoded = BASE64.encode(json.as_bytes());
    format!("/* {CONTINUOUS_REGISTER}:{job_id}:{encoded} */ SELECT 1 AS registered")
}

/// Encode remote continuous input push.
pub fn encode_continuous_push(job_id: &str, batches: &[RecordBatch]) -> FlightResult<String> {
...
pub fn encode_bounded_window(
    ...
    let spec_json = serde_json::to_string(spec).expect("window spec serializes");
...
fn decode_window_spec(encoded: &str) -> FlightResult<WindowExecutionSpec> {
    let bytes = BASE64
        .decode(encoded)
        .map_err(|e| FlightError::transport(format!("invalid window spec encoding: {e}")))?;
    serde_json::from_slice(&bytes)
        .map_err(|e| FlightError::transport(format!("invalid window spec json: {e}")))
}

fn encode_batches_ipc(batches: &[RecordBatch]) -> FlightResult<String> {
    if batches.is_empty() {
    ...
    let mut buffer = Vec::new();
    {
        let mut writer = StreamWriter::try_new(&mut buffer, &schema)
            .map_err(|e| FlightError::transport(format!("ipc encode failed: {e}")))?;
        for batch in batches {
            writer
                .write(batch)
                .map_err(|e| FlightError::transport(format!("ipc write failed: {e}")))?;
        }
        writer
            .finish()
            .map_err(|e| FlightError::transport(format!("ipc finish failed: {e}")))?;
    }
    Ok(BASE64.encode(buffer))
}

fn decode_batches_ipc(encoded: &str) -> FlightResult<Vec<RecordBatch>> {
    if encoded.is_empty() {
        return Ok(Vec::new());
    }
    let bytes = BASE64
        .decode(encoded)
        .map_err(|e| FlightError::transport(format!("invalid batch ipc encoding: {e}")))?;
    let cursor = Cursor::new(bytes);
    let reader = StreamReader::try_new(cursor, None)
        .map_err(|e| FlightError::transport(format!("ipc decode failed: {e}")))?;
    reader
        .collect::<Result<Vec<_>, _>>()
        .map_err(|e| FlightError::transport(format!("ipc read failed: {e}")))
}
