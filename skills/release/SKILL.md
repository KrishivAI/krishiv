...
| `crates/krishiv-connect-client/Cargo.toml` | `version` |
...
sed -i 's/^version = ".*"/version = "'"${VERSION}"'"/' crates/krishiv-connect-client/Cargo.toml
...
git add Cargo.toml Cargo.lock k8s/helm/krishiv/Chart.yaml \
  ...
  python/krishiv-ge/pyproject.toml crates/krishiv-connect-client/Cargo.toml \
