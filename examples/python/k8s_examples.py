...
async def main():
    coordinator_url = "http://127.0.0.1:50051"
    import os
    os.environ["KRISHIV_REMOTE_EXEC"] = "1"
    os.environ["KRISHIV_MODE"] = "distributed"
    os.environ["KRISHIV_COORDINATOR"] = coordinator_url
    ...
    df_batch = session.sql("SELECT user_id, sum(amount) as total_amount FROM transactions GROUP BY user_id")
    # Wait, grouping on unbounded requires a stream or windows, let's just do simple projection
    ...
    async for b in df_batch.execute_stream_async():
        res.append(b.to_arrow().to_pydict())
        break
    ...
    try:
        async for b in df_stream.execute_stream_async():
            print(f"Received streaming result: {b.to_arrow().to_pydict()}")
            count += 1
            if count >= 5:
                break
    ...
    print("Successfully completed both Batch and Streaming SQL on Krishiv K8s cluster!")
